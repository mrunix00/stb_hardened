#define STB_VORBIS_IMPLEMENTATION
#define STB_VORBIS_NO_STDIO
#include "stb_vorbis.c"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

unsigned char poc_ogg[] = {
    
};
unsigned int poc_ogg_len = 354;

int main() {
    int chan, samplerate;
    short *output = NULL;

    printf("BUG-stb_vorbis-007: integer overflow in comment_list_length allocation...\n");
    int samples = stb_vorbis_decode_memory(poc_ogg, poc_ogg_len, &chan, &samplerate, &output);
    if (samples >= 0) {
        printf("Unexpected success: got %d samples\n", samples);
        free(output);
    } else {
        printf("Decoder returned error (expected when bug not triggered)\n");
    }
    return 0;
}
